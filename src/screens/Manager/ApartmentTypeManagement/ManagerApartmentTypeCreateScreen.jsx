...
export default function ManagerApartmentTypeCreateScreen({ navigation }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await apartmentTypeService.createApartmentType(
        apartmentTypeData
      );
      if (response.success) {
        Alert.alert("Thành công", "Tạo loại căn hộ thành công!", [
          { text: "OK", onPress: () => navigation.goBack() },
        ]);
      } else {
        Alert.alert(
          "Lỗi",
          response.message || "Không thể tạo loại căn hộ. Vui lòng thử lại."
        );
      }
    } catch (error) {
      console.log("Error creating apartment type:", error);
      Alert.alert("Lỗi", "Không thể tạo loại căn hộ. Vui lòng thử lại.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <ModernScreenWrapper
      title="Tạo loại căn hộ mới"
      subtitle="Nhập thông tin loại căn hộ"
      headerColor="#2C3E50"
    >
