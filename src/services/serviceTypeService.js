...
export const createServiceType = async (serviceTypeData) => {
  try {
    const response = await httpClient.post(ENDPOINTS.SERVICE_TYPES, serviceTypeData);
    ...
    return {
      success: false,
      message: error.response?.data?.message || 'Có lỗi xảy ra khi tạo loại dịch vụ'
    };
  }
};
...
export const getAllServiceTypes = async (params = {}) => {
  try {
    const response = await httpClient.get(ENDPOINTS.SERVICE_TYPES_GET_ALL, { params });
    ...
    return {
      success: false,
      message: error.response?.data?.message || 'Có lỗi xảy ra khi lấy danh sách loại dịch vụ'
    };
  }
};
...
export const getServiceTypeById = async (serviceTypeId) => {
  try {
    const response = await httpClient.get(`${ENDPOINTS.SERVICE_TYPES_GET_BY_ID}/${serviceTypeId}`);
    ...
    return {
      success: false,
      message: error.response?.data?.message || 'Có lỗi xảy ra khi lấy loại dịch vụ theo ID'
    };
  }
};
...
export const updateServiceType = async (serviceTypeId, serviceTypeData) => {
  try {
    const response = await httpClient.patch(`${ENDPOINTS.SERVICE_TYPES_UPDATE}/${serviceTypeId}`, serviceTypeData);
    ...
    return {
      success: false,
      message: error.response?.data?.message || 'Có lỗi xảy ra khi cập nhật loại dịch vụ'
    };
  }
};
...
export const deleteServiceType = async (serviceTypeId) => {
  try {
    const response = await httpClient.delete(`${ENDPOINTS.SERVICE_TYPES_DELETE}/${serviceTypeId}`);
    ...
    return {
      success: false,
      message: error.response?.data?.message || 'Có lỗi xảy ra khi xóa loại dịch vụ'
    };
  }
};
