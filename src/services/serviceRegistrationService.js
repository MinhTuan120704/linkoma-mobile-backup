...
export const createServiceRegistration = async (serviceRegistrationData) => {
  try {
    const response = await httpClient.post(ENDPOINTS.SERVICE_REGISTRATIONS, serviceRegistrationData);
    ...
    return {
      success: false,
      message: error.response?.data?.message || 'Có lỗi xảy ra khi tạo đăng ký dịch vụ'
    };
  }
};
...
export const getAllServiceRegistrations = async (params = {}) => {
  try {
    const response = await httpClient.get(ENDPOINTS.SERVICE_REGISTRATIONS_GET_ALL, { params });
    ...
    return {
      success: false,
      message: response.data?.message || 'Lấy danh sách đăng ký dịch vụ thất bại'
    };
  } catch (error) {
    console.error('Get all service registrations error:', error);
    return {
      success: false,
      message: error.response?.data?.message || 'Có lỗi xảy ra khi lấy danh sách đăng ký dịch vụ'
    };
  }
};
...
export const getServiceRegistrationById = async (serviceRegistrationId) => {
  try {
    const response = await httpClient.get(`${ENDPOINTS.SERVICE_REGISTRATIONS_GET_BY_ID}/${serviceRegistrationId}`);
    ...
    console.error('Get service registration by ID error:', error);
    return {
      success: false,
      message: error.response?.data?.message || 'Có lỗi xảy ra khi lấy đăng ký dịch vụ theo ID'
    };
  }
};
...
export const updateServiceRegistration = async (serviceRegistrationId, serviceRegistrationData) => {
  try {
    const response = await httpClient.patch(`${ENDPOINTS.SERVICE_REGISTRATIONS_UPDATE}/${serviceRegistrationId}`, serviceRegistrationData);
    ...
    console.error('Update service registration error:', error);
    return {
      success: false,
      message: error.response?.data?.message || 'Có lỗi xảy ra khi cập nhật đăng ký dịch vụ'
    };
  }
};
...
export const deleteServiceRegistration = async (serviceRegistrationId) => {
  try {
    const response = await httpClient.delete(`${ENDPOINTS.SERVICE_REGISTRATIONS_DELETE}/${serviceRegistrationId}`);
    ...
    console.error('Delete service registration error:', error);
    return {
      success: false,
      message: error.response?.data?.message || 'Có lỗi xảy ra khi xóa đăng ký dịch vụ'
    };
  }
};
