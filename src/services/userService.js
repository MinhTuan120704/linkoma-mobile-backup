...
export const getAllUsers = async (params = {}) => {
  try {
    ...
    Alert.alert(
      "Lỗi",
      error.response?.data?.message || "Có lỗi xảy ra khi lấy danh sách user",
      [{ text: "OK" }]
    );
    return {
      success: false,
...
export const createUserWithEmail = async (email) => {
  try {
    const response = await httpClient.post(ENDPOINTS.USERS_CREATE_WITH_EMAIL, {
      email,
    });
    ...
    console.log("Create user with email error:", error);
    Alert.alert(
      "Lỗi",
      error.response?.data?.message || "Có lỗi xảy ra khi tạo user với email",
      [{ text: "OK" }]
    );
    return {
      success: false,
...
export const getUserByEmail = async (email) => {
  try {
    const response = await httpClient.get(
      `${ENDPOINTS.USERS_GET_BY_EMAIL}/${email}`
    );
    ...
    Alert.alert(
      "Lỗi",
      error.response?.data?.message || "Có lỗi xảy ra khi lấy user theo email",
      [{ text: "OK" }]
    );
    return {
      success: false,
      message:
        ...
        "Có lỗi xảy ra khi lấy user theo email",
    };
  }
};
...
export const getUserById = async (userId) => {
  try {
    ...
    return {
      success: false,
      message:
        error.response?.data?.message || "Có lỗi xảy ra khi lấy user theo ID",
    };
  }
};
...
export const updateUser = async (userId, userData) => {
  try {
    ...
    return {
      success: false,
      message:
        error.response?.data?.message || "Có lỗi xảy ra khi cập nhật user",
    };
  }
};
